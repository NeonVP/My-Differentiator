...
void LatexDtor( Latex_t *latex ) {
    ...
    system( "xelatex tex/main.tex -o tex/main.pdf" );
}
...
void DifferentiatorAddOrigExpression( Differentiator_t *diff, int order ) {
    ...
    for ( int i = 1; i <= order; i++ ) {
        ...
        LATEX_PRINT( "\\begin{math} \n" );
        LATEX_PRINT( "f^{(%d)}(x) = ", i );
        NodeToLatex( diff->diff_tree->root, latex_file );
...
void DifferentiatorAddTaylorSeries( Differentiator_t *diff, char var, int order ) {
    ...
    NodeToLatex( diff->taylor_tree->root, diff->latex.tex_file );
    PRINT_O;
    LATEX_PRINT( "\\end{math} \n" );
